...
def gather_data():
    ...
    iceberg_penguin_groups = dict(((i, []) for i in game.get_all_icebergs()))
...
def send_with_acceleration(my_iceberg, another_iceberg, msg="", check_amount_0=False):
    ...
    while speed < 8:
        ...
        if check_amount_0 and amount > 0:
            break
        if net > abs(amount) + 1:
            send_penguins(my_iceberg, another_iceberg, abs(amount) + 1,
                          msg + ", (speed " + str(speed) + ")")
            # print(((turns_till_arrival - speed) // speed) + speed + 1)
            if speed != 1:
                add_to_accelerate_dict.append((my_iceberg, another_iceberg, speed // 2))
            return abs(amount) + 1, True
...
def attack_with_help(current, my_iceberg, attack_iceberg, amount=None, msg=""):
    ...
    for i in range(current + 1, len(my_icebergs)):
        ...
        total_net = sum(helper_net_list) + my_net
        ...
        elif total_net + my_net > enemy_amount:
            send_penguins(my_iceberg, attack_iceberg, my_net, msg + ", attack with help (initiator)")
            for helper, help_amount in zip(helper_list, helper_net_list):
                send_penguins(helper, attack_iceberg, help_amount, "attack with help (helper)")
        else:
            return False
        return True
    return False
...
def real_do_turn():
    global my_icepital, my_icebergs, my_icebergs_min, game, send_to_icepital_and_boom_boom
    #
    ...
    if game.turn < 50:
        ...
        for neutral in game.get_neutral_icebergs():
            if penguin_amount(neutral) > 0:
                my_icebergs_count += 1
        if my_icebergs_count <= 2 and len(enemy_icebergs) >= 4:
            send_to_icepital_and_boom_boom = True
    if send_to_icepital_and_boom_boom:
    ...
    for i in range(len(my_icebergs)):
        ...
        for enemy in game.get_enemy_icebergs():  # type: Iceberg
            ...
            farthest_group.sort(key=lambda pg: pg.turns_till_arrival)
            if farthest_group:
        ...
        # 2 nearest to icepital
        for iceberg in all_icebergs_from_icepital[1:3]:
            if iceberg not in all_my_icebergs_set and iceberg not in neutral_icebergs_set:
                my_icebergs_min[my_iceberg] = net
                can_upgrade &= attack_with_help(i, my_iceberg, iceberg, msg="nearest to icepital")
                net = my_icebergs_min[my_iceberg]
        ...
        for enemy, amount, farthest_group, priority in attack_list_by_priority:
            if net > amount + 1:
                turns_till_arrival = my_iceberg.get_turns_till_arrival(enemy)
                total_amount = penguin_amount(enemy)
                if total_amount > 0:
                    continue
                add = 0
                ...
                if net + add > total_amount:
                    send_penguins(my_iceberg, enemy, net, "attack by priority")
                    net = 0
                    break
        #
